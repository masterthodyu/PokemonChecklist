...
- **A sidebar** per "group set" a checklist defines — Home has Generation and Category. Checklists with no group sets (Colosseum included) skip the sidebar entirely rather than leaving a dead gap. With two or more group sets, only the first (Generation, for Home) gets the left sidebar to itself; everything after it (Category) stacks into a second sidebar on the right instead of both being crammed into one long left-hand column.
